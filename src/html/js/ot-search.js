setTimeout(async () => {
    ...
    if (!searchBar) {
        let tempInt = setInterval(() => {
            searchBar = document.querySelector('[data-a-target="nav-search-input"]');
            if (searchBar) {
                startSearch();
                    ...
                    for (const key in searchResults) {
                        if (Object.hasOwnProperty.call(searchResults, key)) {
                            ...
                            if (keyData) switch (key) {
                                case "channels":
                                    keyData.forEach(async elmnt => {
                                        if (elmnt.stream) insertDivs[0].innerHTML += `
                                        ...
                                        `;
                                            ...
                                            if (elmnt.lastBroadcast && elmnt.lastBroadcast.startedAt) {
                                                let lastStreamTxt = "";
                                                let lastStreamDate = new Date(elmnt.lastBroadcast.startedAt);
                                                let currentDate = new Date();
    
                                                let diffMs = currentDate - lastStreamDate;
                                                ...
                                                switch (true) {
                                                    case diffYears >= 1:
                                                        lastStreamTxt = `${diffYears.toFixed(0)} years`;
                                                    break;
                                                
                                                    case diffMonths >= 1:
                                                        lastStreamTxt = `${diffMonths.toFixed(0)} months`;
                                                    break;
    
                                                    case diffWeeks >= 1:
                                                        lastStreamTxt = `${diffWeeks.toFixed(0)} weeks`;
                                                    break;
    
                                                    case diffDays >= 1:
                                                        lastStreamTxt = `${diffDays.toFixed(0)} days`;
                                                    break;
    
                                                    case diffHours >= 1:
                                                        lastStreamTxt = `${diffHours.toFixed(0)} hours`;
                                                    break;
    
                                                    case diffMinutes >= 1:
                                                        lastStreamTxt = `${diffMinutes.toFixed(0)} minutes`;
                                                    break;
    
                                                    case diffSeconds >= 1:
                                                        lastStreamTxt = `${diffSeconds.toFixed(0)} seconds`;
                                                    break;
                                                }
    
                                                insertDivs[0].innerHTML += `
                                                    <div class="search-result tw-flex tw-full-width tw-mg-t-1 tw-mg-b-1" href="/${elmnt.login}">
                                                        <a class="search-result__image tw-avatar">
                                                            <img src="${elmnt.profileImageURL}">
                                                        </a>
                                                        <div class="search-result__info">
                                                            <a class="search-result__title">
                                                                <h4>${elmnt.displayName}</h4>
                                                            </a>
                                                            <div class="search-result__subtitle">${elmnt.followers.totalCount} followers</div>
                                                            <div class="search-result__subtitle">Last streamed <a href="/videos/${elmnt.lastBroadcast.id}">${lastStreamTxt} ago</a></div>
                                                        </div>
                                                    </div>
                                                `;
                                            } else insertDivs[0].innerHTML += `
                                                <div class="search-result tw-flex tw-full-width tw-mg-t-1 tw-mg-b-1" href="/${elmnt.login}">
                                                    <a class="search-result__image tw-avatar">
                                                        <img src="${elmnt.profileImageURL}">
                                                    </a>
                                        ...
                                        insertDivs[1].innerHTML += `
                                            <div class="search-result tw-flex tw-full-width tw-mg-t-1 tw-mg-b-1" href="/directory/category/${elmnt.slug}">
                                                ...
                                                <div class="search-result__info">
                                                    ...
                                                    <div class="search-result__subtitle">${elmnt.viewersCount ? elmnt.viewersCount : "0"} viewers</div>
                                                </div>
                                            </div>
                                        `;
                                    });
